...
class Article < Content
  ...
  def merge_articles(merged_article)
    # merge text
    self.body = self.body + merged_article.body
    # merge comments
    self.comments = self.comments + merged_article.comments
    self.save
    Article.destroy(merged_article.id)
  end
